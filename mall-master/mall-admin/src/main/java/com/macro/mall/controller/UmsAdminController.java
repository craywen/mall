...
public class UmsAdminController {
    ...
    public CommonResult login(@RequestBody UmsAdminLoginParam umsAdminLoginParam, BindingResult result) {
        String token = adminService.login(umsAdminLoginParam.getUsername(), umsAdminLoginParam.getPassword(),umsAdminLoginParam.getVerificationCode());
        if (token == null) {
            return CommonResult.validateFailed("用户名或密码错误");
        }else if(token.equals("yzmcw")){
            return CommonResult.validateFailed("验证码错误");
        }
        //登录成功，移除验证码缓存
        defaultCacheEngine.remove("VALIDATE_CODE");
